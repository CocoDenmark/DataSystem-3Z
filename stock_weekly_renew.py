...
base_dir = 'G:\\Filesys\\stock\\trade\\No_Rehabilitation\\data\\weekly\\current\\'
error_file = open('G:\\Filesys\\stock\\trade\\No_Rehabilitation\\error_file\\fail_weekly_renew_'+today+'.txt','wb')
...
for code in code_list:
	...
	d = w.wsd(code, "pre_close,open,high,low,close,volume,amt,dealnum,chg,pct_chg,swing,vwap,adjfactor,close2,close3,turn,free_turn,last_trade_day,rel_ipo_chg,rel_ipo_pct_chg,maxupordown,trade_status,susp_reason", today, today, "Period=W;Fill=Previous", "showblank=-1")
...
for c in renew_dict:
	renew_file.write(c+'\r\n')
renew_file.close()
body = body + '-----STOCK-TRADE-WEEKLY--No Rehabilitation-----\r\n' + 'Total: ' + str(len(code_list)) + '\r\n' + 'Succeeded: ' + str(sum) + '\r\n' + 'Failed: ' + str(len(fail_list)) + '\r\n' + '\r\n'

# here begins the trade data (forward rehabilitation) download process by setting the destinate dir
base_dir = 'G:\\Filesys\\stock\\trade\\Forward_Rehabilitation\\data\\weekly\\current\\'
error_file = open('G:\\Filesys\\stock\\trade\\Forward_Rehabilitation\\error_file\\fail_weekly_renew_'+today+'.txt','wb')
...
for code in code_list:
	...
	d = w.wsd(code, "pre_close,open,high,low,close,volume,amt,dealnum,chg,pct_chg,swing,vwap,adjfactor,turn,free_turn,trade_status,susp_reason,maxupordown", today, today, "Period=W;Fill=Previous;PriceAdj=F", "showblank=-1")
...
renew_file.close()
error_file.close()
body = body + '-----STOCK-TRADE-WEEKLY--Forward Rehabilitation-----\r\n' + 'Total: ' + str(len(code_list)) + '\r\n' + 'Succeeded: ' + str(sum) + '\r\n' + 'Failed: ' + str(len(fail_list)) + '\r\n' + '\r\n'


# here begins the trade data (backward rehabilitation) download process by setting the destinate dir
base_dir = 'G:\\Filesys\\stock\\trade\\Backward_Rehabilitation\\data\\weekly\\current\\'
error_file = open('G:\\Filesys\\stock\\trade\\Backward_Rehabilitation\\error_file\\fail_weekly_renew_'+today+'.txt','wb')
...
for code in code_list:
	...
	d = w.wsd(code, "pre_close,open,high,low,close,volume,amt,dealnum,chg,pct_chg,swing,vwap,adjfactor,turn,free_turn,trade_status,susp_reason,maxupordown", today, today, "Period=W;Fill=Previous;PriceAdj=B", "showblank=-1")
...
for c in renew_dict:
	renew_file.write(c+'\r\n')
renew_file.close()
body = body + '-----STOCK-TRADE-WEEKLY--Backward Rehabilitation-----\r\n' + 'Total: ' + str(len(code_list)) + '\r\n' + 'Succeeded: ' + str(sum) + '\r\n' + 'Failed: ' + str(len(fail_list)) + '\r\n' + '\r\n'


# the download and write-into-file process for stock-hold data

base_dir = 'G:\\Filesys\\stock\\hold\\data\\weekly\\current\\'
error_file = open('G:\\Filesys\\stock\\hold\\error_file\\fail_weekly_renew_'+today+'.txt','wb')
...
for code in code_list:
	wind_data = list()
	d = w.wsd(code, "total_shares,free_float_shares,float_a_shares,share_restricteda,share_totala,share_restrictedb,share_totalb,share_h,share_oversea,share_totaltradable,share_totalrestricted,share_nontradable,share_ntrd_prfshare,share_rtd_state,share_rtd_statejur,share_rtd_subotherdomes,share_rtd_domesjur,share_rtd_inst,share_rtd_domesnp,share_rtd_subfrgn,share_rtd_frgnjur,share_rtd_frgnnp,holder_top10pct,holder_top10quantity,holder_top10liqquantity,holder_controller,holder_name", today, today, "Period=W;order=0;Fill=Previous", "showblank=-1")
	wind_data.append(d)
	d = w.wsd(code, "holder_quantity,holder_pct,holder_sharecategory", today, today, "Period=W;order=1;Fill=Previous")
	wind_data.append(d)
	d = w.wsd(code, "holder_liqname", today, today, "Period=W;order=0;Fill=Previous")
	wind_data.append(d)
	d = w.wsd(code, "holder_liqquantity,holder_liqsharecategory,holder_num,holder_avgnum,holder_avgpct,holder_havgpctchange,holder_qavgpctchange,holder_havgchange,holder_qavgchange,holder_avgpctchange", today, today, "Period=W;order=1;shareType=1;Fill=Previous")
	wind_data.append(d)
...
encoding = 'utf-8' 
today = time.strftime('%Y-%m-%d', time.localtime(time.time())) 
subject = '[DATABASE WEEKLY RENEW - stock]  ' + today
